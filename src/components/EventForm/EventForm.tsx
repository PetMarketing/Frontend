...
import { Field, Form, Formik, ErrorMessage as Error, FormikHelpers } from 'formik';
...
export default function EventForm({ eventId }: IEventFormProps) {
    ...
    const submitHandler = async (values: IEventfForm, actions: FormikHelpers<IEventfForm>) => {
        ...
        try {
            const res = await fetch(`${baseURL}/buy-ticket`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div className={styles.eventForm}>
            {isLoading ? (
                ...
                <>
                    <h2 className={getClsNames(styles.title, [dela.className])}>I want to go!</h2>
                    ...
                    <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={(values, actions) => submitHandler(values, actions)}>
                        <Form className={styles.form}>
                            <div className={getClsNames(styles.inputContainer, [styles.floatingLabel])}>
                            ...
                            <div className={getClsNames(styles.inputContainer, [styles.floatingLabel])}>
                                ...
                                <Error name='phone'>{error => <span className={styles.error}>{error}</span>}</Error>
                            </div>
